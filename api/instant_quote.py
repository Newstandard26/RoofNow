"""Vercel Python serverless function: GET /api/instant_quote.

The RoofNow consumer endpoint: address in, instant Good/Better/Best estimate
with a confidence verdict out. Reuses the existing measurement engine
(``measure_address``) and adds the pricing + confidence layer
(``roofwall.quote``).
...
Returns the instant quote as JSON. Falls back to deterministic demo data when
GOOGLE_MAPS_API_KEY is unset (flagged low-confidence by the quote engine).

Rate limiting mirrors /api/measure: live mode hits Google's paid APIs.
"""
...
class handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):  # noqa: N802 - BaseHTTPRequestHandler API
        ...
        try:
            report = measure_address(
                address=address, lat=lat, lng=lng, waste_pct=waste
            )
            quote = build_quote(report)
            self._send(200, quote, {**rl_headers, "Cache-Control": "public, max-age=60"})
